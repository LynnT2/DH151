...
function readCSV(path){
	Papa.parse(path, {
		...
		complete: function(data) {
			console.log(data);
			csvdata = data; //put the data in a global variable

			lastdate = csvdata.meta.fields[csvdata.meta.fields.length-1]; //get a last date and put it in a global variable
			createSidebarButtons();
			mapCSV(lastdate); //call the mapCSV function to map the data
		}
	});
}
	...
	csvdata.data.forEach(function(item,index){
		...
		if(item.Lat != undefined){
			...
			.on('mouseover',function(){
					...
					markers.addLayer(marker) //show data on hover
			}
		});
